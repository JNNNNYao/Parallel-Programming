...
int* image;
// time
struct timespec total_start, total_end, total_temp;
...
void* task(void* arg_void_ptr){
    int* ID = static_cast<int*>(arg_void_ptr);
    ...
    clock_gettime(CLOCK_MONOTONIC, &t_end[*ID]);
    if ((t_end[*ID].tv_nsec - t_start[*ID].tv_nsec) < 0) {
        t_temp[*ID].tv_sec = t_end[*ID].tv_sec-t_start[*ID].tv_sec-1;
        t_temp[*ID].tv_nsec = 1000000000 + t_end[*ID].tv_nsec - t_start[*ID].tv_nsec;
    } else {
        t_temp[*ID].tv_sec = t_end[*ID].tv_sec - t_start[*ID].tv_sec;
        t_temp[*ID].tv_nsec = t_end[*ID].tv_nsec - t_start[*ID].tv_nsec;
    }
    threads_time[*ID] += t_temp[*ID].tv_sec + (double) t_temp[*ID].tv_nsec / 1000000000.0;
    pthread_exit(NULL);
}
...
int main(int argc, char** argv) {
    ...
    t_start = (struct timespec*)malloc(num_threads*sizeof(struct timespec));
    t_end = (struct timespec*)malloc(num_threads*sizeof(struct timespec));
    t_temp = (struct timespec*)malloc(num_threads*sizeof(struct timespec));
    threads_time = (double*)malloc(num_threads*sizeof(double));
    ...
    if ((total_end.tv_nsec - total_start.tv_nsec) < 0) {
        total_temp.tv_sec = total_end.tv_sec-total_start.tv_sec-1;
        total_temp.tv_nsec = 1000000000 + total_end.tv_nsec - total_start.tv_nsec;
    } else {
        total_temp.tv_sec = total_end.tv_sec - total_start.tv_sec;
        total_temp.tv_nsec = total_end.tv_nsec - total_start.tv_nsec;
    }
    total = total_temp.tv_sec + (double) total_temp.tv_nsec / 1000000000.0;
    printf("%lf", total);
    for(int i = 0; i < num_threads; i++){
        printf(" %lf", threads_time[i]);
    }
