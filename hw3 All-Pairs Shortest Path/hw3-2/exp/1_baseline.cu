...
void input(char* infile) {
    ...
    int remainder = V % B_FACTOR;
    pad_V = (remainder == 0)? V: V + (B_FACTOR - remainder);    // make V divisible by B_FACTOR for coalescing
...
__global__ void cal_phase1(int *D, int Round, int V) {
    ...
    for (int k = 0; k < B_FACTOR; ++k) {
        for (int i = 0; i < B_FACTOR/blockDim.y; i++) {
            if (D[(block_round + (i + pos_i)) * V + (block_round + pos_j)] > D[(block_round + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block_round + pos_j)])
                D[(block_round + (i + pos_i)) * V + (block_round + pos_j)] = D[(block_round + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block_round + pos_j)];
        }
        __syncthreads();
    }
}
...
__global__ void cal_phase2(int *D, int Round, int V) {
    ...
    for (int k = 0; k < B_FACTOR; ++k) {
        for (int i = 0; i < B_FACTOR/blockDim.y; i++) {
            if (D[(block_round + (i + pos_i)) * V + (block + pos_j)] > D[(block_round + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block + pos_j)])
                D[(block_round + (i + pos_i)) * V + (block + pos_j)] = D[(block_round + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block + pos_j)];
            if (D[(block + (i + pos_i)) * V + (block_round + pos_j)] > D[(block + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block_round + pos_j)])
                D[(block + (i + pos_i)) * V + (block_round + pos_j)] = D[(block + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block_round + pos_j)];
        }
        __syncthreads();
    }
}
...
__global__ void cal_phase3(int *D, int Round, int V) {
    ...
    for (int k = 0; k < B_FACTOR; ++k) {
        for (int i = 0; i < B_FACTOR/blockDim.y; i++) {
            if (D[(block_row + (i + pos_i)) * V + (block_col + pos_j)] > D[(block_row + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block_col + pos_j)])
                D[(block_row + (i + pos_i)) * V + (block_col + pos_j)] = D[(block_row + (i + pos_i)) * V + (block_round + k)] + D[(block_round + k) * V + (block_col + pos_j)];
        }
        __syncthreads();
    }
}
