...
__global__ void cal_phase1(int *D, int Round, int V) {
    ...
    for (int k = 0; k < B_FACTOR; ++k) {
        for (int i = 0; i < B_FACTOR/(blockDim.y); i++) {
            if (shared[pos_i][pos_j] > shared[pos_i][k] + shared[k][pos_j])
                shared[pos_i][pos_j] = shared[pos_i][k] + shared[k][pos_j];
            if (shared[(1 + pos_i)][pos_j] > shared[(1 + pos_i)][k] + shared[k][pos_j])
                shared[(1 + pos_i)][pos_j] = shared[(1 + pos_i)][k] + shared[k][pos_j];
            if (shared[(2 + pos_i)][pos_j] > shared[(2 + pos_i)][k] + shared[k][pos_j])
                shared[(2 + pos_i)][pos_j] = shared[(2 + pos_i)][k] + shared[k][pos_j];
            if (shared[(3 + pos_i)][pos_j] > shared[(3 + pos_i)][k] + shared[k][pos_j])
                shared[(3 + pos_i)][pos_j] = shared[(3 + pos_i)][k] + shared[k][pos_j];
        }
        __syncthreads();
    }
...
__global__ void cal_phase2(int *D, int Round, int V) {
    ...
    for (int k = 0; k < B_FACTOR; ++k) {
        for (int i = 0; i < B_FACTOR/(blockDim.y); i++) {
            if (row[pos_i][pos_j] > pivot[pos_i][k] + row[k][pos_j])
                row[pos_i][pos_j] = pivot[pos_i][k] + row[k][pos_j];
            if (row[(1 + pos_i)][pos_j] > pivot[(1 + pos_i)][k] + row[k][pos_j])
                row[(1 + pos_i)][pos_j] = pivot[(1 + pos_i)][k] + row[k][pos_j];
            if (row[(2 + pos_i)][pos_j] > pivot[(2 + pos_i)][k] + row[k][pos_j])
                row[(2 + pos_i)][pos_j] = pivot[(2 + pos_i)][k] + row[k][pos_j];
            if (row[(3 + pos_i)][pos_j] > pivot[(3 + pos_i)][k] + row[k][pos_j])
                row[(3 + pos_i)][pos_j] = pivot[(3 + pos_i)][k] + row[k][pos_j];
            // be careful with the inequality
            if (col[pos_i][pos_j] > col[pos_i][k] + pivot[k][pos_j])
                col[pos_i][pos_j] = col[pos_i][k] + pivot[k][pos_j];
            if (col[(1 + pos_i)][pos_j] > col[(1 + pos_i)][k] + pivot[k][pos_j])
                col[(1 + pos_i)][pos_j] = col[(1 + pos_i)][k] + pivot[k][pos_j];
            if (col[(2 + pos_i)][pos_j] > col[(2 + pos_i)][k] + pivot[k][pos_j])
                col[(2 + pos_i)][pos_j] = col[(2 + pos_i)][k] + pivot[k][pos_j];
            if (col[(3 + pos_i)][pos_j] > col[(3 + pos_i)][k] + pivot[k][pos_j])
                col[(3 + pos_i)][pos_j] = col[(3 + pos_i)][k] + pivot[k][pos_j];
        }
        __syncthreads();
    }
...
__global__ void cal_phase3(int *D, int Round, int V) {
    ...
    for (int k = 0; k < B_FACTOR; ++k) {
        if (i_j_0 > row[(pos_i)][k] + col[k][pos_j])
            i_j_0 = row[(pos_i)][k] + col[k][pos_j];
        if (i_j_1 > row[(1 + pos_i)][k] + col[k][pos_j])
            i_j_1 = row[(1 + pos_i)][k] + col[k][pos_j];
        if (i_j_2 > row[(2 + pos_i)][k] + col[k][pos_j])
            i_j_2 = row[(2 + pos_i)][k] + col[k][pos_j];
        if (i_j_3 > row[(3 + pos_i)][k] + col[k][pos_j])
            i_j_3 = row[(3 + pos_i)][k] + col[k][pos_j];
    }
