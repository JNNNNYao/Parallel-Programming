...
__global__ void cal_phase3(int *D, int Round, int V, int y_offset) {
    if (blockIdx.x == Round || (blockIdx.y+y_offset/B_FACTOR) == Round) {
        return;
    }
    ...
    int block_row = blockIdx.y * B_FACTOR + y_offset;
    int block_round = Round * B_FACTOR;
...
void block_FW(int B) {
    ...
    int* d_Dist[2];
    int y_offset = ((pad_V/B_FACTOR) % 2 == 0)? pad_V / 2: (pad_V/B_FACTOR/2+1) * B_FACTOR;
    ...
    #pragma omp parallel
    {
        ...
        cudaMalloc(&d_Dist[cpu_thread_id], pad_V * pad_V * sizeof(int));
        cudaMemcpy(d_Dist[cpu_thread_id], Dist, pad_V * pad_V * sizeof(int), cudaMemcpyHostToDevice);
        dim3 num_threads(B_FACTOR, 1024/B_FACTOR);  // 64, 16
        dim3 num_blocks(pad_V/B_FACTOR, y_offset/B_FACTOR);

        int round = pad_V / B;
        for (int r = 0; r < round; ++r) {
            /* Phase 1*/
            cal_phase1 <<<1, num_threads>>> (d_Dist[cpu_thread_id], r, pad_V);

            /* Phase 2*/
            cal_phase2 <<<pad_V/B_FACTOR, num_threads>>> (d_Dist[cpu_thread_id], r, pad_V);

            /* Phase 3*/
            cal_phase3 <<<num_blocks, num_threads>>> (d_Dist[cpu_thread_id], r, pad_V, y_offset*cpu_thread_id);

            cudaDeviceSynchronize();
            #pragma omp barrier
            
            // if the pivot block and pivot-row of next round is calculate by other device: copy
            if (cpu_thread_id == 1 && (r+1) < y_offset/B_FACTOR) {
                cudaMemcpy(d_Dist[1]+(r+1)*B_FACTOR*pad_V, d_Dist[0]+(r+1)*B_FACTOR*pad_V, B_FACTOR*pad_V*sizeof(int), cudaMemcpyDeviceToDevice);
            }
            else if(cpu_thread_id == 0 && (r+1) >= y_offset/B_FACTOR) {
                cudaMemcpy(d_Dist[0]+(r+1)*B_FACTOR*pad_V, d_Dist[1]+(r+1)*B_FACTOR*pad_V, B_FACTOR*pad_V*sizeof(int), cudaMemcpyDeviceToDevice);
            }
        }
        if (cpu_thread_id == 0) {
            cudaMemcpy(Dist, d_Dist[0], y_offset*pad_V*sizeof(int), cudaMemcpyDeviceToHost);
        }
        else {
            cudaMemcpy(Dist+y_offset*pad_V, d_Dist[1]+y_offset*pad_V, ((pad_V-y_offset)*pad_V)*sizeof(int), cudaMemcpyDeviceToHost);
        }
    }
}
