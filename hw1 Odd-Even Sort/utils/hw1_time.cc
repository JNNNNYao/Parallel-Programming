...
int main(int argc, char** argv) {
    ...
    double IO = 0, Comm = 0, CPU = 0, Total = 0;
    ...
    CPU = Total - IO - Comm;
    // if(rank == 0){
    //     printf("%lf %lf %lf %lf", IO, Comm, CPU, Total);
    // }
    double sum_IO, sum_Comm, sum_CPU, sum_Total;
    MPI_Reduce(&IO, &sum_IO, 1, MPI_DOUBLE, MPI_SUM, 0, comm_world);
    MPI_Reduce(&Comm, &sum_Comm, 1, MPI_DOUBLE, MPI_SUM, 0, comm_world);
    MPI_Reduce(&CPU, &sum_CPU, 1, MPI_DOUBLE, MPI_SUM, 0, comm_world);
    MPI_Reduce(&Total, &sum_Total, 1, MPI_DOUBLE, MPI_SUM, 0, comm_world);
    if(rank == 0){
        printf("%lf %lf %lf %lf", sum_IO/numtasks, sum_Comm/numtasks, sum_CPU/numtasks, sum_Total/numtasks);
    }
