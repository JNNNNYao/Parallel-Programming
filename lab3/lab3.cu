...
#define MASK_Y 5
#define SCALE 8
#define NUM_THREAD 256 // maximum value is 1024
#define SIZE 16
...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    y = blockIdx.x * SIZE + threadIdx.x;
    x = blockIdx.y * SIZE + threadIdx.y;
    for (i = 0; i < MASK_N; ++i) {
...
int main(int argc, char** argv) {
    ...
    unsigned count = height * width * channels;
    cudaMalloc((void **)&device_s, count * sizeof(unsigned char));
    cudaMalloc((void **)&device_t, count * sizeof(unsigned char));

    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(device_s, host_s, count * sizeof(unsigned char), cudaMemcpyHostToDevice);

    /* Hint 3 */
    // acclerate this function
    dim3 num_thread(SIZE, SIZE);
    dim3 num_block(height/SIZE, width/SIZE);
    sobel<<<num_block, num_thread>>>(device_s, device_t, height, width, channels);
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, device_t, count * sizeof(unsigned char), cudaMemcpyDeviceToHost);
